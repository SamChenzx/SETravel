...
static NSString *const resetCellIdentifier = @"resetCellIdentifier";
...
- (NSArray *)caseTypeNames {
    if (!_caseTypeNames) {
        _caseTypeNames = @[@"QA手动", @"RD手动", @"UI自动化"];
    }
...
- (NSArray *)executeTypeNames {
    if (!_executeTypeNames) {
        _executeTypeNames = @[@"manual_qa", @"manual_rd", @"automated_ui"];
    }
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (!cell) {
            cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:resetCellIdentifier];
